...
},
{
  ...
  const { data: domainLists, refresh: refreshSearch, pending: searchLoading, error: searchError } =
    await useAsyncData('get-domain', () =>
      $fetch(authHost + 'api/links?q=' + encodeURIComponent(keyword) + "&skip=" + (page - 1) + "&limit=" + size + "&ispStatus=" + chooseValue)
    );
...
async function onSearchDomain(domain) {
  ...
  const webPage = `{
    ...
    "@type": "WebApplication",
    "name": "VBlacklist",
    "url": "https://vblacklist.com",
    "description": "Công cụ kiểm tra trạng thái website trên tất cả nhà mạng Việt Nam. Phát hiện ISP chặn, cảnh báo downtime và dịch vụ AntiBlock bảo vệ website 24/7.",
  ...
  const title = "vBlackList – Kiểm Tra Tình Trạng Website Trên Tất Cả Nhà Mạng Việt Nam";
  const description = "Công cụ kiểm tra trạng thái website nhanh nhất Việt Nam: phát hiện bị chặn ISP, kiểm tra domain realtime, xem dữ liệu theo nhà mạng, lịch sử quét và dịch vụ bảo vệ website khỏi chặn.";
  const config = useRuntimeConfig()
   useHead({
    ...
    meta: [
      ...
      { property: 'og:url', content: config.public.host },
      { name: 'title', content: title },
      { name: 'description', content: description },
      { property: 'og:title', content: title },
      { property: 'og:description', content: description },
    ],
    link: [{ rel: 'canonical',content: config.public.host }],
    script: [
      {
        innerHTML: webPage,
  ...
  <main class="p-2">
    ...
    <section class="mb-4">
      <AtomBoxContainer class="flex-1">
        ...
        <AtomBoxContainerHeader>Danh sách domain mới nhất</AtomBoxContainerHeader>
        <div>
          <div class="mb-4">
